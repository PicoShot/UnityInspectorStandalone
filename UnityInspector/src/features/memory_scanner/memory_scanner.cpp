...
void MemoryScanner::Render()
{
    ...
    if (ImGui::Begin("Memory Scanner", &Config::settings.memoryScanner.showWindow))
    {
        ...
        ImGui::Separator();
        ImVec4 statusColor = ImVec4(0.0f, 1.0f, 0.0f, 1.0f);
        if (scanInProgress) statusColor = ImVec4(1.0f, 0.8f, 0.0f, 1.0f);
        else if (!statusText.empty() && statusText.find("error") != std::string::npos) statusColor = ImVec4(1.0f, 0.0f, 0.0f, 1.0f);
        ...
        if (ImGui::BeginTable("Results", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY | ImGuiTableFlags_Sortable))
        {
            ...
            for (size_t i = 0; i < currentResults.size(); ++i)
            {
                ...
                if (ImGui::Selectable(valStr.c_str(), isSelected, ImGuiSelectableFlags_SpanAllColumns))
                {
    ...
    for (const auto& assembly : UR::assembly)
    {
        ...
        for (const auto& klass : assembly->classes)
        {
            ...
            for (const auto& field : klass->fields)
            {
                ...
                if (!ReadStaticFieldValue(field->address, actualType, value))
                    continue;
...
void MemoryScanner::ScanUnityObjectFields(std::vector<ScanField>& out)
{
    ...
    for (void* obj : allObjects)
    {
        ...
        if (UT::String* nameStr = nullptr;
            Helper::SafeGetName(reinterpret_cast<UnityObject*>(obj), nameStr) && nameStr)
        {
    ...
    void* currentClass = klass;
        while (currentClass)
        {
                ...
                mono ? "mono_class_get_fields" : "il2cpp_class_get_fields", currentClass, &iter)))
            {
                ...
                std::string typeNameStr = typeName ? typeName : "unknown";

                if (TypeNameMatchesSearchType(typeNameStr))
                {
                    ...
                    if (!isPrimitive && !isString && !isArray && !isDelegate)
                    {
                        ...
                        if (fieldClass)
                        {
                            ...
                            if (isValueType)
                            {
                                ...
                                ScanObjectInstance((char*)obj + offset, fieldClass, out, visited, depth + 1, childName);
                            }
                            else
                            {
                                void* refObj = nullptr;
                                if (Helper::SafeReadPointer(obj, offset, refObj) && refObj)
                                {
...
bool MemoryScanner::ReadFieldValue(const ScanField& field, void* outValue) const
{
    if (field.isStatic)
    {
        return ReadStaticFieldValue(field.fieldHandle, field.valueType, outValue);
    }
    else
    {
...
bool MemoryScanner::ReadStaticFieldValue(void* fieldHandle, ScanValueType type, void* outValue) const
{
...
bool MemoryScanner::ReadInstanceFieldValue(void* obj, int offset, ScanValueType type, void* outValue) const
{
    ...
    case ScanValueType::Int:
    {
        ...
        if (!SafeRead(obj, offset, v))
            return false;
        *reinterpret_cast<int64_t*>(outValue) = static_cast<int64_t>(v);
        return true;
    }
    case ScanValueType::Long:
    {
        ...
        if (!SafeRead(obj, offset, v))
            return false;
        *reinterpret_cast<int64_t*>(outValue) = v;
        return true;
    }
    case ScanValueType::Float:
    {
        ...
        if (!SafeRead(obj, offset, v))
            return false;
        *reinterpret_cast<float*>(outValue) = v;
        return true;
    }
    case ScanValueType::Double:
    {
        ...
        if (!SafeRead(obj, offset, v))
            return false;
        *reinterpret_cast<double*>(outValue) = v;
        return true;
    }
    case ScanValueType::Bool:
    {
        ...
        if (!SafeReadBool(obj, offset, v))
            return false;
        *reinterpret_cast<bool*>(outValue) = v;
        return true;
    }
    }
    return false;
...
bool MemoryScanner::CompareValueWithTarget(const void* value, ScanValueType actualType) const
{
    ...
    case ScanValueType::Int:
    {
        ...
        if (actualType == ScanValueType::Int)
            return *reinterpret_cast<const int64_t*>(value) == target;
    ...
    case ScanValueType::Float:
    {
        ...
        if (actualType == ScanValueType::Float)
            return *reinterpret_cast<const float*>(value) == target;
        return false;
    }
    case ScanValueType::Double:
    {
        ...
        if (actualType == ScanValueType::Double)
            return *reinterpret_cast<const double*>(value) == target;
        return false;
    }
    case ScanValueType::Bool:
    {
        ...
        if (actualType == ScanValueType::Bool)
            return *reinterpret_cast<const bool*>(value) == target;
        return false;
    }
    }
    return false;
...
int MemoryScanner::CompareValueWithPrevious(const void* currentValue, const ScanField& field) const
{
    ...
    case ScanValueType::Long:
    {
        int64_t cur = *reinterpret_cast<const int64_t*>(currentValue);
    ...
    case ScanValueType::Float:
    {
        float cur = *reinterpret_cast<const float*>(currentValue);
    ...
    case ScanValueType::Double:
    {
        double cur = *reinterpret_cast<const double*>(currentValue);
    ...
    case ScanValueType::Bool:
    {
        int cur = *reinterpret_cast<const bool*>(currentValue) ? 1 : 0;
...
void MemoryScanner::OpenResultInInspector(const ScanField& result)
{
